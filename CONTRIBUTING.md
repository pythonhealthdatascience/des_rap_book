...
**Note:** It does flag several quarto defaults / built-ins as problems, so we acknowledge this tool can be difficult to work with. For example:
...
* Ensure [role="img"] elements have alternative text (`.bi-github`).
* Ensure <img> elements have alternative text or a role of none or presentation, for images that are generated by code.

However, we have found it occassionally useful - for identifying images that are missing alternative text, and elements (like buttons) where there is insufficient contrast between the button background and overlaid text.
