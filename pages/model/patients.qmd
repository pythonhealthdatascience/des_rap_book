...
Instead of manually creating a list of seeds for the generators - such as with patterns like [`run_number`, `run_number + 1`] - we can use NumPy's `SeedSequence`.

Provide a single starter seed (e.g., `run_number`), then call `SeedSequence.spawn()` to generate a list of independent seeds. In this case, we just create a list with one seed (`spawn(1)`).
