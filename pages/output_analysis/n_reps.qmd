...
This page continues on from: [Length of warm-up](length_warmup.qmd).

> *Entity generation → Entity processing → Initialisation bias → Performance measures → Replications → Length of warm-up → Number of replications*
...
The simulation being used is that from the [Replications](replications.qmd) page - but the `Parameters` class has been updated based on the determined [length of warm-up](length_warmup.qmd) on the previous page, and a data collection period of 2 weeks (20,160 minutes).
...
::: {.callout-note title="`sim-tools` v1.0.0 workaround" collapse="true"}
