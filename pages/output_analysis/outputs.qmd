...
<!--TODO: add section after preparation before metrics on time-weighted calculations, like in the pydesrapmms notebook -->
...
class Model:
    ...
    def run(self):
        ...
        # At simulation end, update time-weighted statistics for any#<<
        # resources in use by accounting for the time from the last#<<
        # event up to the simulation finish.#<<
...
For this approach, we create a new class called `MonitoredResource`. This is explained line-by-line above, where we describe how to calculate time-weighted utilisation.

This class is similar, except that we are monitoring `area_n_in_queue` (`len(self.queue) * time_since_last_event`) instead of `area_resource_busy`. These changes are highlighted.
...
In `Model`, we make three changes (same as we did for time-weighted utilisation):
...
class Model:
    ...
    def run(self):
        ...
        # At simulation end, update time-weighted statistics for any#<<
        # requests in queue by accounting for the time from the last#<<
        # event up to the simulation finish.#<<
        self.doctor.update_time_weighted_stats()#<<
