...
We could use an "audit" approach, where we have a method that runs at regular intervals to find the value of each metric at that point (as implemented in the nurse visit simulation example, mentioned at the end of this page).

However, as we have set up our model to record list of times and performance measures, we can actually make use of these directly instead.
...
```{python}
#| echo: false
#| file: replications_resources/parameters.py
```
...
```{python}
#| echo: false
#| file: replications_resources/summary_stats.py
```
