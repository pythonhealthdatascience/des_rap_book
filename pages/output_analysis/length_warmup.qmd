...
We will re-use the `calc_utilisation` function to calculate utilisation at each timepoint. To enable this, we need to add two new arguments:

* `groups` - previously we just grouped by resource, but we now want to also group by replication (as we are processing a full results table containing multiple replications).
* `summarise` - previously we just returned the overall utilisation for a single run, but now we want to return the full dataframe with utilisation values at each timepoint.
...
time_series_inspection <- function(result, file_path, warm_up = NULL) {
  ...
  metrics[[5L]] <- result[["patients_in_system"]] |>
    rename(patients_in_system = count)
...
```{.r}
  ...
  # Patients in system at each timepoint
  metrics[[5L]] <- result[["patients_in_system"]] |>
    rename(count = patients_in_system)
```
...
The selection of a warm-up period is **subjective**. In this example, we have suggested a value of 40,000, but this choice could be changed subject to discussion and evaluation of other metrics.
