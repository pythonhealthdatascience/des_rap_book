...
For this approach, we need to collect data on each performance measure as the simulation runs. There are two main was to do this, and both require a few code changes.

If you only need metrics that are already returned at the end of a run (for example, mean wait time and mean time in system from the patient results), Option 1 is simpler. However, if you also want time‑weighted measures such as utilisation, queue length, or patients in system, you’ll need to extend the model to extract these during the run.

<br>

**Option 1. Use data recorded as events occurred**.

During the simulation, many performance measures are already recorded and stored in lists, along with their corresponding event times. These provide a detailed, event‑driven history of how the system changed over time. To analyse the warm‑up period, we extract and process these lists, since not all of this detailed information is returned automatically at the end of a run.

::: {.callout title="▶&nbsp;&nbsp;View option 1: data recorded as events occurred" collapse="true"}
...
<!--TODO: switch to just using auditor for python... but keep an explanation for future reference of WHY option 1 is a bit rubbish in comparison! If do that, remove text from n_reps about not keeping warm-up model changes... and could switch time record back to int rather than keeping all in a list... when explain why audit class, say why class not method in model-->

**Option 2. Audit at regular intervals**.

Alternatively, we can monitor the model at regular, fixed time intervals (rather than each event) to see how performance measures change over time. This approach produces evenly spaced time series, making it easier to compare runs.

We'd suggest using this approach, as you can keep all code for determining warm-up separate from the model functions and classes, so it keeps it cleaner!

:::{.callout title="▶&nbsp;&nbsp;View option 2: audit at regular intervals" collapse="true"}

**Acknowledgements:** This approach is adapted from Tom Monks (2024) Lab 6 Output Analysis in [HPDM097 - Making a difference with health data](https://github.com/health-data-science-OR/stochastic_systems) (MIT Licence).
...
```{python}
def run_warmup_analysis(audit_interval, data_collection_period, number_of_runs):
    """
