...
<!--TODO: move collect_during_run into parameter, and move validation there too-->
...
class Runner:
    ...
    def __init__(self, param, collect_during_run=False):#<<
        ...
        if self.collect_during_run and self.param.warm_up_period > 0:#<<
            raise ValueError(#<<
                "collect_during_run=True is intended for determining the ",#<<
                "appropriate warm-up period, so warm-up length must be 0. ",#<<
                f"Received warm_up_period={self.param.warm_up_period}."#<<
            )#<<
    ...
    def _cumulative_tw_mean(self, time_list, area_list, metric):#<<
        ...
        -------#<<
        pd.DataFrame#<<
            Dataframe with two columns: time and metric.#<<
        """#<<
...
<!--TODO: python on 20,000 at the moment...-->
...
param = Parameters(warm_up_period=0,
                   ...
                   number_of_runs=5)
runner = Runner(param, collect_during_run=True)
results = runner.run_reps()["during_run"]
```

#### Metrics from `patient` (not time-weighted): mean wait time and mean time in system
...
```{python}
fig = plot_cumulative_mean(results, "wait_time", 1500)
fig.show()
```

```{python}
fig = plot_cumulative_mean(results, "time_in_system", 1500)
fig.show()
```

```{python}
# Calculate overall cumulative mean
result_overall = results["wait_time"].sort_values("time")
result_overall["overall_cumulative_mean"] = (
    result_overall["cumulative_mean_wait_time"].expanding().mean()
)

# Plot cumulative mean from each run
fig1 = px.line(data_frame=results["wait_time"],
               x="time",
               y="cumulative_mean_wait_time",
               line_group="run")
fig1.update_traces(line_color="lightblue")

# Add overall cumulative mean to plot
fig2 = px.line(data_frame=result_overall, x="time", y="overall_cumulative_mean")
fig1.add_traces(list(fig2.select_traces()))

# Add vertical line for warm-up period
fig1.add_vline(x=1500, line_color="red", line_dash="dash",
               annotation_text="Suggested warm-up",
               annotation_font_color="red",
               annotation_position="top right")

# Amend axis labels and plot appearance
fig1.update_layout(
    xaxis_title="Run time (minutes)",
    yaxis_title="Cumulative mean wait time",
    template="plotly_white"
)
fig1.show()
```
...
```{python}
# Calculate overall cumulative mean
result_overall = results["time_in_system"].sort_values("time")
result_overall["overall_cumulative_mean"] = (
    result_overall["cumulative_mean_time_in_system"].expanding().mean()
)

# Plot cumulative mean from each run
fig1 = px.line(data_frame=results["time_in_system"],
               x="time",
               y="cumulative_mean_time_in_system",
               line_group="run")
fig1.update_traces(line_color="lightblue")
...
fig1.update_layout(
    xaxis_title="Run time (minutes)",
    yaxis_title="Cumulative mean time in system",
    template="plotly_white"
)
fig1.show()
```
