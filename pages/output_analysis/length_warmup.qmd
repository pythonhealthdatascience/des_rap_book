...
We have a new `time_series_inspection` function used for this analysis, which is explained line-by-line below.
...
If you've already built warm-up logic into your model (as we have done), make sure the **warm-up period is set to 0** for this assessment (`warm_up_period = 0L`).

As mention above, we have used multiple replications (`number_of_runs = 5L`) and a long run length (`data_collection_period = 100000L`).
...
time_series_inspection(
  result = results,
  file_path = file.path("length_warmup_resources", "length_warmup.png"),
  warm_up = 15000L
)
```
...
If you track additional performance metrics like resource utilisation or queue length, extend the function by adding these metrics to the `metrics` list.
