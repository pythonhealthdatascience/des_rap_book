...
We'll demonstrate steps for input modelling below using the **synthetic arrival data** from the [nurse visit simulation](../intro/examples.qmd) (not based on real-world data). In this case, we already know which distributions to use (as we sampled from them to create our synthetic data!). However, the steps still illustrate how you might select distributions in practice with real data.

## 🔧 Set-up

Click here to download the arrival data. You can save it into a folder (e.g. `data/`).
...
def inspect_histogram(series, x_lab, bin_width=1):
    ...
    bin_labels = [f"{x_lab}: {bin_edges[i]}–{bin_edges[i+1]}" for i in range(nbins)]
    fig.update_traces(
