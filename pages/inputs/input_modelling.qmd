...
<!--DIFF: Doesn't import knitr or tidyr-->
```{r, message=FALSE}
...
```{r}
# Get IAT and service time columns early (with NA dropped)
data_iat <- data %>% drop_na(iat_mins) %>% select(iat_mins)
data_service <- data %>% select(service_mins)

# Plot histograms and CDFs
plotdist(pull(data_iat), histo = TRUE, demp = TRUE)
plotdist(pull(data_service), histo = TRUE, demp = TRUE)
```
...
To implement the targeted approach using `fitdistrplus`...

```{r}
# Fit and run Kolmogorov-Smirnov test on the inter-arrival and service times
distributions <- c("expon", "gamma", "weibull")

fe <- fitdist(data$iat_mins.dropna(), "expon")
fg <- fitdist(data$iat_mins.dropna(), "gamma")
fw <- fitdist(data$iat_mins.dropna(), "weibull")
```
