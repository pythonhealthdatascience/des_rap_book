...
This page has step-by-step instructions for input modelling in Python or R, with inspiration from @Robinson2007 and @Monks2024. For advice on making your input modelling workflow **reproducible** and **sharing data or scripts with sensitive content**, see @sec-input_data.
...
* **Level of detail**. The data must be granular enough for your needs. For example, daily totals may be insufficient if you want to model hourly arrivals (although may still be possible if know distribution - see @sec-input_modelling-options)
...
## Input modelling {#sec-input_modelling-options}
...
<br> 
### Create script
TODO: WHERE to store in repo
:::{.python-content}
TODO: ipynb... touch notebooks/input_modelling.ipynb
:::
...
```{python}
# Plot daily arrivals
fig = inspect_time_series(time_series=data.groupby(by=["ARRIVAL_DATE"]).size(),
...
fig = inspect_time_series(time_series=daily_service,
...
fig = inspect_histogram(series=data["iat_mins"],
                        x_lab="Inter-arrival time (min)")
fig.show()

# Plot histogram of service times
fig = inspect_histogram(series=data["service_mins"],
...
The simplest way to do this is to just use the plotting functions from `distfitrplus`.
...
The `distfitrplus` package does not have a built-in function to automatically fit a large set of distributions in a single command. Instead, we just need to specify a list of candidate distributions.
...
It's good to have an idea of good likely distributions before testing a few. You can do this using `distfitrplus`, testing just those or a whole panel of different distributions.

In this case, we found **exponential** to be best for inter-arrival and service times.

:::

### Parameters

<!--TODO: fetch the parameters for these dists... and then link to sections on parameters from script or file -->
