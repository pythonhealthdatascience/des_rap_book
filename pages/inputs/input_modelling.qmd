...
We'll demonstrate steps for input modelling below using the **synthetic arrival data** from the [nurse visit simulation](../intro/examples.qmd). In this case, we already know which distributions to use (as we sampled from them to create our synthetic data!). However, the steps still illustrate how you might select distributions in practice with real data.
