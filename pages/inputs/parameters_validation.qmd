...
{{< include parameters_validation_resources/param_function.py >}}
...
{{< include parameters_validation_resources/param_function.R >}}
...
{{< include parameters_validation_resources/ParamClass.py >}}
...
:::{.python-content}
```{python}
def check_param_names(param, param_function):
    """
...
:::{.python-content}
```{python}
def model(param, param_function):
    """
...
validate_param <- function(parameters) {
  transfer_prob <- parameters$transfer_prob
  if (transfer_prob < 0 || transfer_prob > 1) { 
    stop(paste0(
...
# Example usage
# Run model() invalid transfer_prob - raises an error
params <- param_function(transfer_prob = 1.4)
try(model(params))
```
:::
...
ParamClass <- R6Class(
  public = list(
    ...
    validate_param = function() { #<<
      if (self$transfer_prob < 0 || self$transfer_prob > 1) { #<<
        stop(paste0( #<<
          "transfer_prob must be between 0 and 1, but is: ", self$transfer_prob #<<
        )) #<<
      } #<<
    } #<<
  )
