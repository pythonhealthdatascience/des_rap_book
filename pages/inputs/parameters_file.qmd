...
* **🔑 Principles when importing**
* **🗃️ Importing from a CSV**
* **🗂️ Importing from JSON**
...
# Import parameters
parameter_file = "parameters_file_resources/example_parameters.json"
...
param <- fromJSON("parameters_file_resources/example_parameters.json")
str(param)
```
...
These same principles apply when importing parameters from files.

## 🗃️ Importing from a CSV

We recommend using **structured parameter objects** rather than passing raw CSV data directly to your simulation functions because:
...
<!--
you could use a similar approach to that described for csv

however, we'll show another way you could do it

remember this book just guidance... always lots of ways!

in this example, we just import, gets stored as dictionary, just as above.

```{python}
import json

# Import parameters
filename = "parameters_file_resources/example_parameters.json"
with open(filename, "r", encoding="utf-8") as f:
    file_content = json.load(f)

# Extract the simulation parameters
sim_params = file_content["simulation_parameters"]
```

now, what about validation? above import to class was because...

well, validation is explained on next page. one of the ways is using a class we make called lockeddict. another is validation performed when distributions are created. find out more...
