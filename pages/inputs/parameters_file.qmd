...
The following `# pylint: disable` lines simply turn off some linting warnings that would otherwise appear in this page's examples. If you want to learn more about linting, see the [linting page](../style_docs/linting.qmd).
...
```{r}
library(jsonlite)  # nolint: undesirable_function_linter
...
subprocess.run([
    "pandoc",
    "parameters_file_resources/csv_data_dictionary.md",
    "-o", "parameters_file_resources/csv_data_dictionary.pdf"
], check=False)
subprocess.run([
    "pandoc",
    "parameters_file_resources/json_data_dictionary.md",
    "-o", "parameters_file_resources/json_data_dictionary.pdf"
], check=False)
```

### CSV data dictionary example

<iframe src="parameters_file_resources/csv_data_dictionary.pdf" width="100%" height="500px" style="border:1px solid #dee2e6"></iframe>

### JSON data dictionary example

<iframe src="parameters_file_resources/json_data_dictionary.pdf" width="100%" height="500px" style="border:1px solid #dee2e6"></iframe>

## 📥 Methods for importing parameters

As explained in the [parameters from script page](parameters_script.qmd), there are two key principles for managing parameters effectively in simulation models:

1. **Group parameters into a dedicated object**.
2. **Pass this object explicitly to your model**.

If you haven't already, see the [script page](parameters_script.qmd) for more detail on why this explicit grouping and passing is important.
...
```{r}
params <- list(
  adult = list(interarrival = 5, service_time = 10),
  child = list(interarrival = 3, service_time = 7),
  distribution = list(mean = 0, sd = 1)
)
...
#' @export
create_parameters <- function(json_file, number_of_runs = 10) {
...
#' @export
create_parameters <- function(csv_file, number_of_runs = 10) {
    df <- read.csv(csv_file)
    lst <- as.list(df$value)
    names(lst) <- paste(df$patient, df$metric, sep="_")
...
create_parameters <- function(csv_file, number_of_runs = 10) {
    df <- read.csv(csv_file)
    lst <- lapply(split(df, df$patient), function(subdf) {
      setNames(as.list(subdf$value), subdf$metric)
    })
    lst$number_of_runs <- number_of_runs
    return(lst)
}
...
As described above, classes also allow you to "lock down" the allowed parameters and add validation checks during initialisation and any changes. For examples of how to add validation when using a class, see the [parameter validation page](parameters_validation.qmd).

::: {.r-content}
This requires each parameter to be a field, rather than storing all parameters in a list within the class. In R, we cannot dynamically add these fields when initialising, TODO.
:::
...
#' Store simulation parameters, combining those loaded from a JSON file with
#' values defined in the class.
CreateParameters <- R6Class("CreateParameters",
  ...
  public = list(
    ...
    #' @param json_file Path to JSON file containing simulation parameters.
    #' @param number_of_runs Number of simulation runs (default is 10).
    initialize = function(json_file, number_of_runs = 10) {
      json_content <- jsonlite::fromJSON(json_file)
      self$params <- json_content$simulation_parameters
      self$params$number_of_runs <- number_of_runs
    }
  )
)


param <- CreateParameters$new(
  file.path("parameters_file_resources", "example_parameters.json")
)
print(param$params[1:4])
```
:::
...
The previous examples dynamically set attributes based on whatever is present in your input file.

It's worth noting that you don't have to use class attributes for this kind of setup - you could instead maintain a list of required parameter names and feed values to the class accordingly (see the [parameter validation page](parameters_validation.qmd) for details).

Here, we show the explicit attribute approach to demonstrate another way of importing that provides clarity and structure, and because it fundamentally changes the import process compared to the earlier dynamic methods.
