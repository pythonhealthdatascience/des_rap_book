---
title: STARS
date: "2025-10-24T10:55:46+01:00"
---
...
**Definitions** (based on [Benureau and Rougier 2018](https://doi.org/10.3389/fninf.2017.00069)):

::: {.callout-warning title="Re-runnable" icon="false"}
...
::: {.callout-note
 title="Repeatable" icon="false"}

Running the same code with the same inputs in the same environment reliably produces identical outputs each time. For example, setting a random seed.

:::

::: {.callout-tip title="Reproducible" icon="false"}

If all code, data, environment details, and parameters are recorded and published, anyone (including yourself in the future or a different person on a different system) can regenerate the published results by re-creating the documented conditions. This verifies code is working as expected and increases trust

:::

::: {.callout-important title="Reusable" icon="false"}

Code can be adapted and used in new contexts. This saves time and increases impact.

:::

::: {.callout-caution title="Replicable" icon="false"}

New code based on described methods produces consistent results. This gives confidence in results: their validity and reliability.
...
![Reproduction recommendations.](stars_resources/reproduction_wheel.png){.lightbox width=48%}
![Troubleshooting recommendations.](stars_resources/troubleshooting_wheel.png){.lightbox width=48%}
