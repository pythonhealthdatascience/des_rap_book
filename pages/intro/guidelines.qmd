...
format:
  html:
    css: guidelines_resources/guidelines_style.css
...
This page introduces the guidelines that underpin the development of reproducible discrete-event simulation (DES) models.
...
(1) **STARS Reproducibility Recommendations** from Heather et al. (2025).

(2) **A framework for reproducible analytical pipelines (RAP)** from the NHS RAP Community of Practice.

Each page demonstrates how to put these guidelines into practice. Use the links below (🔗) to jump straight to relevant pages for each item in the frameworks.
...
Some topics in this guide (e.g. input modelling, parameter validation, and output analysis) are **not directly addressed by these guidelines**. However, because this book aims to be a full end-to-end RAP for DES, these steps are included to reflect **everything researchers typically do when developing DES models in practice**.
...
| List dependencies and versions | <span class="section-link">[Environments](../setup/environment.qmd)</span> |
| **Running the model** |
| Provide code for all scenarios and sensitivity analyses ⭐ | <span class="section-link">[Scenario and sensitivity analysis](../experiments/scenarios.qmd)</span> |
| Ensure model parameters are correct ⭐ | <span class="section-link">[Full run](../experiments/full_run.qmd)</span> |
...
| Include code to generate the tables, figures, and other reported results ⭐ | <span class="section-link">[Producing tables and figures](../experiments/tables_figures.qmd)</span> |
: {tbl-colwidths="[60,40]"}
...
| Save outputs to a file | <span class="section-link">[Scenario and sensitivity analysis](../experiments/scenarios.qmd)</span> <span class="section-link">[Producing tables and figures](../experiments/tables_figures.qmd)</span> |
| Avoid excessive output files | <span class="section-link">[Scenario and sensitivity analysis](../experiments/scenarios.qmd)</span> |
| Address large file sizes | <span class="section-link">[Scenario and sensitivity analysis](../experiments/scenarios.qmd)</span> |
: {tbl-colwidths="[60,40]"}
...
The two guidelines used were chosen for their **strong relevance to healthcare discrete-event simulation** and their focus on **reproducibility**.

There are other guidelines - though with a different emphasis - that are likewise relevant when creating a simulation model. These are mentioned under "Sharing your simulation model" - "4. Follow guidelines" - on the [sharing and archiving](../sharing/archive.qmd#follow-guidelines) page.
