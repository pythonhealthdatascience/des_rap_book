...
* Understand why **importing parameters from a file** may actually often be preferable.
...
This code should form part of your package code rather than standalone scripts; if you need a reminder of where package code lives in the repository, see the earlier page on [structuring your DES RAP package](/pages/guide/setup/package.qmd).
...
* ✅ **Fewer inputs**. If all parameters are in one object, then we can just pass that as a single input to our model, reducing the number of arguments we need to pass.
...
There are two implementation options: **list** or **function**.
...
The **most robust** approach is to use a **function** to manage your parameters.

* ✅ **Functions** make it easy to create variations for different scenarios, since you simply change the inputs when you define a new scenario. For example, you can create a new scenario by only specifying the parameter you want to change, while all other parameters remain at their default values.
...
### Using a single function for all parameters
...
Alternatively, parameters can be split into logical groups (e.g., patient type, parameter type), with each group having its own function. These groups are then combined into a single parameter set.

For simulations with a large number of parameters, this approach simplifies input management. It also enables validation to be performed for each individual function.
...
While the approaches on this page define parameters directly within scripts, in most cases **storing [parameters in external files](parameters_file.qmd) is preferable**. This is particularly true when:

* Running **multiple scenarios** or parameter sets.
* Conducting **sensitivity analyses** .
* Collaborating with **non-programmers** who need to modify parameters.
* Need to **separate parameters from code** for version control.
* Want to **share simulations** with different parameter versions (e.g., real vs synthetic data).

Parameters in scripts work well for:

* **Simple models** with few parameters
* **Development and testing** where you're frequently adjusting values
* **Tutorial or teaching** contexts where everything in one place aids understanding
* **Quick prototypes** before formalising the parameter structure

For production analyses and complex models, the approaches shown on the [Parameters from file](parameters_file.qmd) page are generally recommended.

The M/M/s example model linked below uses parameters within a script, as it a tutorial example with few parameters, keeping everything visible for learning. The stroke pathway simulation uses parameters from a file due to its greater complexity.
...
* **Try out grouping and passing parameters**, using examples from above.

:::

* Create at least **one variant scenario** by overriding one or more parameters.
