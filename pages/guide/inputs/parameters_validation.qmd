...
```{python}
#| echo: false
# pylint: disable=undefined-variable,used-before-assignment,no-member
...
This page describes how to validate functions introduced on:
...
Parameter validation is about catching problems in your inputs before they quietly corrupt your results. In DES models, even a small typo or out‑of‑range value can create plausible‑looking but wrong outputs, because the simulation will usually run without complaining. Good validation acts as risk management: it turns silent failures into explicit errors with clear messages.
...
::: {.callout-note title="Direct import avoids this problem"}

If you use **direct import to model functions** (see [Parameters from file](parameters_file.qmd#direct-import-to-model-functions)), this problem does not occur. The function signature automatically prevents undefined parameters from being added. If you're using that approach, you can skip this section.

This section applies to approaches where parameters are stored in an intermediate object (list or function output) before being passed to your model.
...
To prevent these silent failures, you should **implement parameter validation within your model functions**.
...
This is based on the arguments of `param_function`. We first use `checkmate::assert_list()` to ensure `param` is a properly formed list with uniquely named elements.

We use the `checkmate` package's `assert_*` functions because they automatically generate concise, informative error messages when something is wrong.
...
check_param_names <- function(param, param_function) {
  # Ensure param is a list with unique named elements
...
check_param_names_json <- function(param, param_file) {
  ...
  missing_names <- setdiff(names(param_function()), names(param))
  extra_names <- setdiff(names(param), names(param_function()))
  if (length(missing_names) > 0L || length(extra_names) > 0L)
...
## Validating parameter values (e.g. range)

You can check that the provided inputs are valid - expected format, range, etc. This can be part of your [assertion checking](../verification_validation/verification_validation.qmd#assertion-checking), which is when you add checks in the model which cause errors if something doesn't look right, as part of model verification. 

::: {.python-content}

Parameter validation can either be:
...
We use `checkmate::assert_number()` to ensure the value is numeric and within the valid range.
...
#'
#' @return NULL, invisibly. Throws an error if parameters are invalid.
#' @export

validate_param <- function(parameters) {
  # Ensure parameters is a list with unique named elements and at 1+ entries
  assert_list(parameters, names = "unique", min.len = 1L, 
              .var.name = "parameters")
  # Ensure transfer_prob is a single numeric value between 0 and 1
  assert_number(
    parameters$transfer_prob,
    lower = 0, upper = 1,
    .var.name = "transfer_prob"
  )
}
```
:::
...
## Why is parameter validation especially important in discrete‑event simulation models?
...
cat(longmcq(c(
  paste0(
    "Because simulations will always crash immediately if a parameter name ",
    "is misspelled."
  ),
  answer = paste0(
    "Because small typos or out‑of‑range values can silently produce ",
    "plausible‑looking but incorrect simulation results."
  ),
  "Because parameter validation automatically makes the simulation run faster."
)))
```
...
::: {.python-content}
* Create a simple set of parameters for a simulation model using a function, dictionary, or class. You can reuse the simple `param_function()`, `ParamClass` and/or `model()` from earlier on this page, or from the [Parameters from script](parameters_script.qmd) or [Parameters from file](parameters_file.qmd) pages.
:::
::: {.r-content}
* Create a simple set of parameters for a simulation model. You can reuse the simple `param_function()` and `model()` from earlier on this page, or from the [Parameters from script](parameters_script.qmd) or [Parameters from file](parameters_file.qmd) pages.
:::
