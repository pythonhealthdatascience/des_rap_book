...
read.csv(file.path(data_path, "example_parameters.csv"))
```
...
On that page, we discussed two implementation options: a **list** or a **function**. When defining parameters within scripts, we recommended using a function. But what about when you're importing parameters from a file?
...
We still recommend using a function.

**Direct list import** (loading the file straight into a named list) is rarely practical.
...
* ❌ Named list can be modified after creation, so validation must be performed in the model functions (ensuring the final set of parameters is being checked). This means that errors are only caught later, rather than immediately at the point of import/change.
...
### Function output: nested or flat list
