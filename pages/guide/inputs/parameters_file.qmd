...
For CSV files, use a simple tabular structure and follow tidy data principles - each row is an observation, each column a variable. Avoid merged cells or stacked headers, as these make data processing difficult. You can use either:
...
csv_content = pd.read_csv(
    os.path.join(data_path, "example_parameters_wide.csv")
)
HTML(to_html_datatable(csv_content))
```
