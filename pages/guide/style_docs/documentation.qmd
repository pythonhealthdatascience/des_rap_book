...
![](documentation_resources/github_actions_badge.png){fig-alt="Screenshot of GitHub actions page, with cursor over 'Create status badge'."}

It will then provide you with the required markdown for your badge.

![](documentation_resources/github_actions_badge2.png){fig-alt="Screenshot of GitHub providing markdown for status badge."}
...
Example from [this book's README](https://github.com/pythonhealthdatascience/des_rap_book) (as of January 2026):

![](documentation_resources/allcontributors.png){fig-alt="Screenshot of all-contributors table."}
...
## What is the primary role of a `README.md` file in a simulation repository?
...
cat(longmcq(c(
  answer = paste0(
    "To provide an entry‑point overview with purpose, usage, and key ",
    "instructions for reproducing and running the project."
  ),
  paste0(
    "To summarise key results and figures so readers don’t need to inspect ",
    "the code."
  ),
  paste0(
    "To document only the development history, such as who worked on the ",
    "project and when."
  )
)))
```
...
## What is a key advantage of having a documentation website in addition to README?
...
cat(longmcq(c(
  paste0(
    "It replaces the need to share the underlying code repository, since ",
    "users can just read the website."
  ),
  answer = paste0(
    "It provides a richer, navigable space (tutorials, methodological notes, ",
    "case studies, API reference) that is easier for users to explore than a ",
    "single long markdown file."
  )
)))
```
