...
* Recognise the utility of CI/CD (Continuous Integration and Continuous Delivery/Deployment) for RAP.
...
CI/CD processes (Continuous Integration and Continuous Delivery/Deployment) are a valuable tool for reproducible analytical pipelines. These processes are typically defined as follows:
...
*It may seem a little odd that CD is used for both continuous delivery and continuous deployment, but this dual use is the standard convention in most CI/CD tooling and documentation.*
...
:::{.python-content}
* If you choose to [structure your work as a package](../setup/package.qmd) and publish it (e.g., on PyPI), you can automate **new version releases**.
:::
:::{.r-content}
* If you choose to [structure your work as a package](../setup/package.qmd) and publish it (e.g., on RUniverse), you can automate **new version releases**.
:::
...
Creating a GitHub action to run your model tests is really helpful, as it helps ensure you are running your tests regularly (e.g., everytime you merge into main). This is important for [regression testing](../verification_validation/verification_validation.qmd#regression-testing), a form of model verification which involves re-running your tests everytime you make changes to ensure those changes haven't introduced new errors.

::: {.python-content}

This simple GitHub Actions workflow will install Python 3.13 and dependencies in a `requirements.txt` file, then run `pytest()`.

> **Note:** We'd recommend using `requirements.txt` for GitHub actions as it is much simpler than working with conda inside CI. Locally, we typically maintain a full `environment.yaml` file for conda (which includes the Python version and all dependencies), and then create a corresponding `requirements.txt` listing the Python packages needed to run the project in GitHub Actions. In other words, `requirements.txt` is a pip-style dependency list derived from the same environment as `environment.yaml`, tailored for use in CI.
...
If you have [structured your research as a package](../setup/package.qmd), you can set up a standard R CMD check workflow in one step with:

```{.r}
usethis::use_github_action("check-standard")
```

This creates a `.github/workflows/R-CMD-check.yaml` file and adds an R-CMD-check badge to your README. You can then customise the generated workflow as needed. For example, the workflow below (adapted from the [r-lib/actions examples](https://github.com/r-lib/actions/tree/v2/examples)) checks your package and runs your tests on Ubuntu:
...
## Why is Continuous Integration (CI) particularly valuable for regression testing of DES models?

```{r}
#| output: asis
#| echo: false
cat(longmcq(c(
  "It guarantees that simulation outputs will never change over time.",
  answer = paste0(
    "It re‑runs your tests automatically when changes are pushed or merged, ",
    "making it easier to detect when a code change unintentionally alters ",
...
## Why might you choose to run CI workflows on multiple operating systems (e.g., Ubuntu, Windows, macOS) for your simulation code?
...
cat(longmcq(c(
  answer = paste0(
    "To ensure that OS‑specific differences in paths, tooling, or ",
    "dependencies do not cause the model or tests to fail on platforms your ",
    "users may use."
  ),
  "To comply with GitHub’s default requirements for public repositories."
)))
```
