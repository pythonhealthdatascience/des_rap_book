...
For the rest of the book, we will use simmer's default activity log output for simplicity. This page is just to show you how you could set up custom logs if you want them.
...
We need to add the following functions from `simmer`: `get_attribute`, `log_` and `set_attribute`.
...
Our argument `verbose` has been replaced by three new arguments: `log_to_console`, `log_to_file` and `file_path`...
