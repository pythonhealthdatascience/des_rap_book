...
![](../inputs/input_data_resources/samples.png){fig-alt="Illustration showing multiple samples being taken from one distribution."}
...
![](distributions_resources/prng_sample.png){fig-alt="Illustration showing that seed is given to PRNG which generates random number stream. Stream and distribution parameters are then fed to sampler which produces samples from distribution."}
...
The [sim-tools](https://github.com/sim-tools/sim-tools) package (developed by Prof. Tom Monks) provides an object-oriented approach to sampling from a variety of statistical distributions.
...
In R, random sampling is typically done with functions from the `stats` package. To ensure reproducibility, set a global random seed with `set.seed()`, using any integer value (e.g., `set.seed(3L)` or `set.seed(123L)`). Then use the appropriate function for your desired distribution.
...
On the [Input modelling](/pages/guide/inputs/input_modelling.qmd) page, we introduced a table of common probability distributions and discussed when each is appropriate for DES. On this page, the relevant sampling functions are shown:

::: {.python-content}

| Distribution | NumPy `Generator` function | `sim_tools` distribution class |
| - | - | - |
| Poisson | `rng.poisson()` | `Poisson()` |
| Exponential | `rng.exponential()` | `Exponential()` |
| Gamma | `rng.gamma()` | `Gamma()` |
| Lognormal | `rng.lognormal()` | `Lognormal()` |
| Weibull | `rng.weibull()` | `Weibull()` |
| Normal | `rng.normal()` | `Normal()` |
| Uniform | `rng.uniform()` | `Uniform()` |

:::

::: {.r-content}

| Distribution | Base R function | `simEd` function |
| - | - | - |
| Poisson | `rpois()` | `vpois()` |
| Exponential | `rexp()` | `vexp()` |
| Gamma | `rgamma()` | `vgamma()` |
| Lognormal | `rlnorm()` | `vlnorm()` |
| Weibull | `rweibull()` | `vweibull()` |
| Normal | `rnorm()` | `vnorm()` |
| Uniform | `runif()` | `vunif()` |
