...
date: "2025-11-06T10:01:59+00:00"
---
...
* Compare **tools** for managing environments.
* Learn methods for switching between python **versions**.
* Use **conda** for dependency management.
* Learn steps to **restore** environments and **troubleshoot** common issues.

:::

::: {.r-content}

* Understand what R **environments** are and why they matter.
* Compare **tools** for managing environments.
* Learn methods for switching between R **versions**.
...
![](environment_resources/time_capsule.png){fig-align="center" width=70% fig-alt="Illustration representing time capsule idea."}
...
![](environment_resources/isolated_env.png){fig-align="center" width=70% fig-alt="Illustration representing isolated environments."}
...
![](environment_resources/collaborate.png){fig-align="center" width=70% fig-alt="Illustration representing collaboration with matching environments."}

## Tools

:::: {.python-content}

In Python, there are several popular tools for environment management. We'd recommend using a tool that can do both:

1. **Install and switch between different versions of Python**
...
* **Default location** - by default, are environments created in the project folder (local) or in a central cache (shared across projects)?
  * **Note:** This isn't too important, as for all these tools, the location can be customised!
* **Dependency files** - does the tool maintain a list of top‑level packages, or a full snapshot of the environment?
  * "**Top-level packages**" are packages that you explicitly request (for example, the names you put in `environment.yaml`).
  * A **snapshot** is a complete pinned list of *all* packages in the environment (including indirect dependencies) with exact versions so the environment can be recreated.
* **Speed**
* **Whether it can build and publish packages** - can the tool build Python packages for distribution? If you are [structuring your research as a package](package.qmd), you may wish to use one tool - or separate tools - for dependency management and creation of your package.

::: {.grey-table}
| Tool | Python versions | Default location | Dependency files | Speed | Can build + publish package? |
| - | - | - | - | - | - |
| `venv` | ❌ No | Project folder | `requirements.txt` *(top-level only)* | ⚡ Fast | ❌ No |
| `conda` | ✅ Yes | Central | `environment.yml` *(top-level or snapshot)* | 🐢 Slow | ❌ No |
| `mamba` | ✅ Yes | Central | `environment.yml` *(top-level or snapshot)* | 🚀 Very fast | ❌ No |
| `poetry` | ⚠️ Partial | Central | `pyproject.toml` *(top-level)* + `poetry.lock` *(snapshot)* | 🐢 Slow | ✅ Yes |
| `uv` | 	✅ Yes | Project folder | `pyproject.toml` *(top-level)* + `uv.lock` *(snapshot)* | 🚀 Very fast | ✅ Yes |
:::
...
A command-line tool for installing, listing, and switching between multiple R versions on Windows, macOS, and Linux.

| **renv**
| 🔗 <https://rstudio.github.io/renv/>
A package for project-specific libraries and lockfiles. It records the exact package versions used in a project (via `renv.lock`), so you can recreate the same environment later or on another machine.

| **rv**
| 🔗 <https://a2-ai.github.io/rv-docs/>
An emerging tool for declarative R environments using `rproject.toml` and `rv.lock`. You declare the R version and all dependencies up front; rv then enforces that version and set of packages for the project.
...
* `default` and `anaconda` - Maintained by Anaconda's engineers and designed for high security and stability.  As of January 2026, organisations with 200 or more employees have to pay to use this channel (excluding students and non-commercial research at universities). Please check [Anaconda's current Terms of Service](https://www.anaconda.com/legal/terms/terms-of-service) for the latest details.
...
::: {.callout-note title="Viewing and setting working directory"}

If you are using Linux, macOS or Git Bash:

```{.bash}
# Show current folder (Linux / macOS)
pwd

# List files to check environment.yaml is here
ls

# If you are not in the right folder, move into it
...
This sets R 4.1.2 as the default R version for new R sessions (including RStudio).

If you later switch R versions in another way (for example, by reinstalling R or changing your PATH) and want rig to control it again, run `rig default <version>` once more (e.g., `rig default 4.1.2`) to set that version as the default again.
...
![](environment_resources/rproject1.png){fig-alt="Screenshot of RStudio New Project Wizard: Create Project."}

Navigate to your project directory, then select "Create Project".

![](environment_resources/rproject2.png){fig-alt="Screenshot of RStudio New Project Wizard: Create Project from Existing Directory."}
...
![](environment_resources/renv_init.png){fig-alt="Screenshot of RStudio Packages pane showing Project Library v.s. User Library."}
...
![](environment_resources/description_vs_lock.png){fig-alt="Illustration showing DESCRIPTION containing 'simmer', whilst renv.lock contains 'simmer 4.4.7', 'Rcpp 1.0.14', and 'magrittr 2.0.3'."}
...
It does not yet include your simulation model code itself; you will create that as a separate package and then add it to this environment as a local install on the [Structuring as a package](package.qmd) page.

To install the environment:

1. In your `des-rap-python/` folder, create an `environment.yaml` file and copy in the text below.
...
The environment below does not yet include your simulation model code itself; you will create that as a separate package and then add it to this environment as a local install on the [Structuring as a package](package.qmd) page.
...
If you want to know the exact versions used, see the [renv.lock](https://github.com/pythonhealthdatascience/des_rap_book/blob/main/renv.lock) file in the `des_rap_book` GitHub repository.
...
Follow the [installation instructions](https://a2-ai.github.io/rv-docs/intro/installation/) for your operating system. For example, on linux:
...
::: {.callout-note title="Using pak for system requirements"}
...
When you return to a project after some time or share it with collaborators, you often need to **recreate the original environment** so that the code runs successfully, produces the same results as before, and can be reliably reused or adapted by others.

::: {.python-content}

If you skip this step and instead use different versions of Python and packages, the code may fail to run altogether or may produce different results from those originally reported.

:::

::: {.r-content}

If you skip this step and instead use different versions of R and packages, the code may fail to run altogether or may produce different results from those originally reported.

:::

This section explains how to recreate an environment.
...
::: {.callout-note title="Alternative approach: time-based snapshots"}
...
After MRAN closed, [**Posit Public Package Manager (P3M)**](https://packagemanager.posit.co/client/#/) became the main free source for R package snapshots. P3M creates daily snapshots of CRAN, Bioconductor and Python's PyPI, which you can use the same way: to get the exact package set for a chosen date. It's limitation however is that it does not archive packages from GitHub and r-universe. For more on P3M, check out this video from [Posit PBC](https://www.youtube.com/@PositPBC):

{{< video https://youtu.be/NwZ4sVjNKlI?si=hb4KG7MT9HMa07ky >}}
...
local({
  ...
  repos["P3M"] <- "https://packagemanager.posit.co/cran/2023-01-17"
  # optional: keep a CRAN fallback
...
::: {.callout-note title="GitHub and r-universe package considerations"}
...
Environment management is important because it lets you **isolate** project dependencies and reliably **recreate** the same setup later for reproducible work and collaboration.

::: {.python-content}

We introduce five main tools for managing environments: **venv, conda, mamba, poetry and uv**.

We go into detail on using **conda/mamba to control Python versions and packages**: install conda/mamba, create an `environment.yaml` file with desired packages, then run:
...
You can then pin the exact package versions installed in the file - for example:
...
We provide the full **des-rap-book environment** specification required to run all examples in the book.

We also describe how to **recreate and troubleshoot environments**: find or reconstruct an environment file, rebuild the environment, and then resolve installation or compatibility errors by adjusting versions or channels and updating the environment file.

:::

::: {.r-content}

We introduce three tools for managing environments: **rig, renv and rv**.

We explain how to use **rig** to manage R versions, for example:
...
We then explain **renv** for package management, with key steps of:

1. Creating an `.Rproj`.
2. Initialising `renv` (`renv::init()`).
3. Create a `DESCRIPTION` file declaring the main dependencies.
4. Installing these (`renv::install()`) (may require changing the snapshot type, as e.g., `implicit` mode will only install them if it can see them in use in your code).
5. Recording the specific package versions installed in an `renv.lock` file (`renv::snapshot()`).

We provide a full **DES RAP Book R environment**, listing all packages need to run the examples in the book.

We also describe how to **recreate and troubleshoot environments**: identify or infer R version and packages, restore from renv.lock or DESCRIPTION, handle time‑based snapshots and GitHub/r-universe packages, and diagnose common problems such as unavailable packages, backdating needs, missing system libraries, legacy user libraries and when Docker or rocker images may be preferable.
...
## Why should you create an isolated environment for each project?
...
cat(longmcq(c(
  paste0(
    "So the code will keep working even if you uninstall Python or R from ",
    "your computer."
  ),
  "So the project will only ever work on the exact same physical computer.",
  answer = paste0(
    "So you can recreate the same versions of the language and packages ",
    "later, ensuring results can be reproduced."
  )
)))
```
...
## When first creating a conda environment, what do we recommend putting in your `environment.yaml`?

```{r}
#| output: asis
#| echo: false
cat(longmcq(c(
  "A full list of every possible package you might ever use in the future.",
  answer = paste0(
    "Just the environment name, channels (e.g. `conda-forge`), and the ",
    "Python version you want to use."
  ),
  paste0(
    "Only your simulation model package and no information about Python or ",
    "channels."
  )
)))
```
...
## After building a Python environment and installing the packages you want, what is the recommended next step for reproducibility?
...
cat(longmcq(c(
  paste0(
    "Delete `environment.yaml` because the environment now lives on your ",
    "machine."
  ),
  answer = paste0(
    "Update `environment.yaml` to pin the exact package versions you are ",
    "using, for example `python=3.13.3` and `simpy=4.1.1`."
  ),
  "Run `conda env export` and ignore any manually written files."
)))
```
...
## What is the primary role of `rig`?
...
cat(longmcq(c(
  answer = paste0(
    "To install, list, and switch between different R versions on your ",
    "machine (e.g. `rig add 4.1.2`, `rig default 4.1.2`)."
  ),
  "To manage project‑specific package libraries and lockfiles.",
  "To build R packages and upload them to CRAN."
)))
```
...
## On this page, why is a `DESCRIPTION` file recommended alongside `renv.lock` for R projects?

```{r}
#| output: asis
#| echo: false
cat(longmcq(c(
  "Because `renv.lock` is optional and never used during restoration.",
  answer = paste0(
    "Because `DESCRIPTION` clearly lists the main packages your project ",
    "depends on, complementing `renv.lock` which records all packages ",
    "and exact versions."
  ),
  paste0(
    "Because `DESCRIPTION` automatically replaces the need for ",
    "`renv::snapshot()`."
  )
)))
```
...
## What is a sensible first step when you want to recreate someone else's environment?
...
cat(longmcq(c(
  paste0(
    "Install whatever latest language and package versions you prefer and ",
    "ignore project files."
  ),
  "Manually edit all scripts to remove any package dependencies.",
  answer = paste0(
    "Look for existing environment files (e.g. `environment.yaml`, ",
    "`renv.lock`, `DESCRIPTION`) and use them, or construct one based on ",
    "the documented versions and packages."
  )
)))
```
