...
* **Initial conditions:** Start the model with some entities or resources already present (e.g., a certain number of patients already waiting). This can be difficult though, as it requires relevant data for determining an appropriate starting state.
...
Added new imports from `dplyr` and `rlang`, along with two additional `simmer` functions: `get_mon_arrivals` and `get_mon_resources`.
...
Also, `verbose` is now set as `FALSE`.
...
## What is initialisation bias in a DES model?
...
cat(longmcq(c(
  ...
  "Bias caused by using wrong time units.",
  answer = "Bias introduced by starting in an empty or atypical state."
)))
```
...
## In which type of system is starting the simulation from an empty state usually acceptable?

```{r}
#| output: asis
#| echo: false
cat(longmcq(c(
  "A hospital ward with chronic bed shortages",
  answer = "A clinic that opens empty at the start of each working day",
  "A crowded emergency department that is busy 24/7"
)))
```
