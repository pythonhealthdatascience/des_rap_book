...
## Why does the workflow separate warm-up and data-collection periods in both Python and R?

```{r}
#| output: asis
#| echo: false
cat(longmcq(c(
  "To make it easier to debug patient-level logs.",
  answer = paste0(
    "To ensure the inital system behaviour does not bias the performance ",
    "measures."
  ),
  "To allow different random number generators in each period.",
  "To reduce the total runtime of the simulation."
)))
```
...
## Why are backlogged (unseen) patients tracked separately from completed patients when computing mean wait times?

```{r}
#| output: asis
#| echo: false
cat(longmcq(c(
  "Because unseen patients never arrive in the system.",
  "Because they always have zero wait time.",
  answer = paste0(
    "Because their waits are censored at the end of the run, and including ",
    "them in the same mean would underestimate true waits for completed ",
    "patients."
  )
)))
```
