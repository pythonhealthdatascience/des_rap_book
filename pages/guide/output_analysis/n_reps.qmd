...
For this analysis, we have a function `run_replications_algorithm()`.

::: {.callout-note collapse="true" title="View replication algorithm code"}

You just need to use `run_replications_algorithm()`. You will find its code at the end of this callout, and the example in the text will show you how to use it.

Behind this function sits a slightly more advanced R construct called an R6 class. R6 is just one way of organising stateful code in R; you can ignore the details if you have never seen it before. In the background, three R6 classes work together to implement the algorithm:

* `WelfordStats` - keeps a running mean, variance and confidence interval using Welford's online algorithm.
* `ReplicationTabuliser` - records the results from each replication so we can inspect how precision improves as we add runs.
* `ReplicationsAlgorithm` - coordinates everything and decides how many replications are needed to reach the desired precision.

The key point: you do not need to interact with these classes directly. In all the examples you will simply:

1. Set up your model parameters.
2. Call `run_replications_algorithm(param = ...)`.
3. Work with the returned results (recommended number of replications and a summary table).

If you are curious about how the object‑oriented machinery works, or you'd like a gentle introduction to R6 for your own projects, check out this video from Posit: <https://youtu.be/3GEFd8rZQgY>.

Otherwise, feel free to treat `run_replications_algorithm()` as a black box and focus on the simulation ideas.
...
```{r}
# Run algorithm
res <- run_replications_algorithm(
  param = create_params(warm_up_period = 10000L,
                        data_collection_period = 10000L),
  metrics = metrics
)
...
```{r}
plot_replication_ci(
  conf_ints = filter(res$summary_table, metric == "mean_wait_time_doctor"),
  yaxis_title = "Mean wait time",
  min_rep = res$nreps[["mean_wait_time_doctor"]]
)
```
...
```{r}
plot_replication_ci(
  conf_ints = filter(res$summary_table, metric == "utilisation_doctor"),
  yaxis_title = "Mean utilisation",
  min_rep = res$nreps[["utilisation_doctor"]]
)
```
...
```{r}
plot_replication_ci(
  conf_ints = filter(res$summary_table, metric == "mean_queue_length_doctor"),
  yaxis_title = "Mean queue length",
  min_rep = res$nreps[["mean_queue_length_doctor"]]
)
```
...
```{r}
plot_replication_ci(
  conf_ints = filter(res$summary_table, metric == "mean_time_in_system"),
  yaxis_title = "Mean time in system",
  min_rep = res$nreps[["mean_time_in_system"]]
)
```
...
```{r}
plot_replication_ci(
  conf_ints = filter(res$summary_table, metric == "mean_patients_in_system"),
  yaxis_title = "Mean patients in system",
  min_rep = res$nreps[["mean_patients_in_system"]]
)
```
...
## Why do we need multiple replications when analysing a DES model?

```{r}
#| output: asis
#| echo: false
cat(longmcq(c(
  "Because replications are required to remove all randomness from the model.",
  "Because each replication uses a different model structure.",
  answer = paste0(
    "Because each run is affected by randomness, and averaging over many ",
    "replications produces a more stable and reliable estimate."
  )
)))
```
...
## What is the core idea behind the confidence interval method for choosing the number of replications?
...
cat(longmcq(c(
  answer = paste0(
    "Increase the number of replications until the confidence interval ",
    "around the mean becomes sufficiently narrow and stable."
  ),
  "Run a fixed, large number of replications regardless of variability.",
  "Keep adding replications until the mean exactly matches a target value."
)))
```
