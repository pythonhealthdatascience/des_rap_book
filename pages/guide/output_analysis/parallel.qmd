...
::: {.callout-warning title="Parallel runs from notebooks"}

Joblib's parallel execution only works reliably on Windows and MacOS with code that lives in importable Python modules. If you define your `Parameters`, `Model`, `Runner` (and related functions) inside a notebook cell instead of a `.py `file, parallel runs (`cores > 1`) can fail with errors such as "Could not pickle the task to send it to the workers" or "Can't pickle class sim_tools.distributions.Exponential...". This happens because worker processes cannot re-import notebook-defined classes and functions, even though external packages like sim_tools are installed correctly.
​
To avoid this, put your simulation code in a module and import it into the notebook (as explained on the [Structuring as a package](/pages/guide/setup/package.qmd) page). If you prefer to keep everything in the notebook, set `cores = 1` to run sequentially and bypass parallel pickling altogether.
​
:::
...
We've add imports from two new pacakages: `future` and `future.apply`.
...
## What is the main goal of adding parallel processing to the replication workflow?

```{r}
#| output: asis
#| echo: false
cat(longmcq(c(
  "To reduce memory usage of each individual replication.",
  answer = (
    "To run multiple replications at the same time, reducing total run time."
  )
)))
```
...
## What does setting `cores = 1` mean?

```{r}
#| output: asis
#| echo: false
cat(longmcq(c(
  "Use one core for the OS and the rest for the simulation.",
  answer = "Use a single‑core, run sequentially.",
...
cat(longmcq(c(
  "Use one core for the OS and the rest for the simulation.",
  "Use a single‑core, run sequentially.",
  answer = "Use all available cores."
)))
```
