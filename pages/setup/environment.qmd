...
However, `renv` does not manage the R version used, so we also mention manual installation of R versions (in @sec-other), `rig` (manages R version but not packages) and `rv` (a new tool that manages both).
...
As `venv` is included with the standard Python library, you do not need to install it. To create a virtual environment, we use the `python -m venv <name>` command. Typically the environment is just named `venv` - and so, we just execute:
...
`venv` cannot control which python version is used, will just use one on system. You can check what this is by running:
...
rig
<!--TODO: windows and mac too, right?-->
:::
