...
There are several tools for managing dependencies in isolated environment. Some of the most popular tools are described below...
...
<!--TODO: describe each, add an illustration with logos from each, add explanation of why use what... In this book, we will use **conda**, as it allows us to **specify a python version**, so each environment we create can have a different specific version of python. This is not possible with venv and poetry, which just manage the package dependencies, and will just use the system python.-->
:::
...
:::{.python-content}
In our repositories for this book, we have used **conda**.
...
The distinction between `Imports` and `Suggests` is more relevant when [constructing your research as an R package](package.qmd), as it will distinguish between those necessary for the core simulation and those for other analysis and tests. <!--TODO: make sure there is a section in packages about this-->
...
<!--TODO: add explanations of other package managers-->
