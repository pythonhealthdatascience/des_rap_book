...
**1. Add package to environment file**. Edit the `environment.yaml` file created in @sec-environment to add the local package. We use the `-e` flag to install it in "editable" mode, which means it will update with any changes to the source code in `simulation/`.
...
* **URL**: Link to your repository. If you don't have one, we'd strongly recommend making one - check out the @sec-version page.
...
* **License**: A license tells others how they can use your code. The `usethis` package makes it easy to add a license: just call the function for your chosen license, for example:
    ...
    For more information, see the @sec-licence page in this book, and the [R Packages book](https://r-pkgs.org/license.html).
...
<!--TODO: write summary focused on just summarising how you can structure it as a package - code in R/, need this file and this file, load with this, run analysis here, etc.-->
