...
In the main project folder, create a folder called `simulation/`. For example, your folder structure might look like:
...
Copy the text below into this file. This provides instructions for building the package. We're using `flit` as our build tool because of its simplicity. The `dynamic` field tells flit to extract the version from our `__init__.py` file.
...
For this section, you will need `roxygen2`, `usethis` and `devtools`. If you used the environment provided on the [environments](environment.qmd) page then you should have these already - but if you do not, make sure to install them! You can check if they are in your environment by running this command from the R console:
...
In the main project folder, create a folder called `R/`. For example, your folder structure might look like:
...
Edit the `environment.yaml` file created on the [environments](environment.qmd) page to add the local package. We use the `-e` flag to install it in "editable" mode, which means it will update with any changes to the source code in `simulation/`.
