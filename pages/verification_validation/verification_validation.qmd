...
<!--TODO: sargent: operational model validation = determining that the model behaves accurately based on its intended purpose. though they also have input data validation and conceptual model validation-->
...
* **Cross-reference checking** - check that links/references/relationships between different pieces of information are correct. For example, that can call required functions, that imports have correct file paths, and that documentation is up-to-date and consistent with the code.
    * This overlaps with another technique for verification: **documentation checking**. That involves checking documentation is complete, correct, consistent and unambiguous.
* **Convention violation checking** - ensure adherence with coding standards.
* **Detailed comparison to specification** - systematically compare the implemented model with the original plans - so checking that the requirements, logic, parameters and outputs as planned in the conceptual model are implemented in the code.
* **Reading the code** - look through the code line-by-line to see if it makes sense and spot any possible issues.
...
<!--
TODO: From robinson book: Black-box validation - determining that the overall model represents the real world with sufficient accuracy for the purpose at hand. The overall heaviour of the model is considered. Two approaches: (A) compare to real world, (B) compare to another model (useful when no real world) - B: compare with other, normally simpler models. One approach is to compare the simulation model against a mathematical model. It is unlikely that a mathematical model is able to predict the outcome fot ehs imulation exactly, otherwise the simulation would probably not have been built. However for the purposes of comparison a mathematical model may be able to give a crude approximation to the outputs of the real system. .. This approach is sometimes referred to as static analysis because it does not (cannot) take into account the fully dynamics of the simulation model. In order to aid comparison it is sometimes useful to simplify the simulation model to the extent that a mathematical model can predict exactly, or at least more exactly, the outcome of the model. For example, making it a deterministic model, where all random events are removed, as can then mathetmactially determine exact outcome.
-->
...
confirming that distributions chosen are appropriate reflections of real system...

from banks book: conduct simple statistical tests of input data for homogeneity, for randomness, and for goodness of fit to assumed distributional forms… then pg 354 has Goodness-of-Fit Tests section (in Input Modelling chapter)

sargent: data validation - accuracy of raw data and any transformation performed on this data

robinson: Data validation - whether data used at any stage are sufficiently accurate

### TODO: Conceptual model validation

sargent one of their four phases of v&v: conceptual model validation - whether the theories and assumptions undelying the conceptual model structure are correct, and whether the model's structure, logic, and mathematical and causal relationships are "reasonable" for the intende dpurpose of the model

robinson: Conceptual model validation - whether content, assumptions and simplifications are sufficiently accurate (i.e. does model have necessary details to meet study objective)

### TODO: Experimentation validation

robinson: ensuring procedures allow accurate results i.e. requirements for removing initialisation bias, run length, replications, sensitivity analysis, and searching of solution space to ensure that learning is maximised and appropriate solutions identified

### TODO: Cross validation

@Zhang2025 - cross validation performed and reported	… comparison across similar modeling studies which deal with the same decision problem was undertaken.
-->
...
TODO: write... look over the types of verification and validation... what can you do? could make a checklist of things could do that would help meet...
