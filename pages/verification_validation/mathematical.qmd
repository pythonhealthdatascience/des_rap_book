...
This page implements mathematical proof of correctness implemented on the [verification and validation page](verification_validation.qmd). It does so within a test ([tests](tests.qmd) page). The tests are run on the model from [Replications](../output_analysis/replications.qmd) (which has two performance measures: total arrivals and mean wait time).

> *Entity generation → Entity processing → Initialisation bias → Performance measures → Replications → Mathematical proof of correctness*
